...
# Nice to have monthly rent
nice_to_have_max_monthly_rent = 1500
nice_to_have_max_monthly_rent_weight = 3

nice_to_have_min_square_feet = 1000
nice_to_have_min_square_feet_weight = 6

or_queries      = %w(amenity:cats_allowed amenity:ski_resort amenity:parking_garage amenity:dogs_allowed)
or_query_weight = [1, 2, 2, 5]
max_score       = or_query_weight.inject(&:+) + nice_to_have_max_monthly_rent_weight + nice_to_have_min_square_feet_weight

or_queries_score = or_queries.each_with_index.map do |or_query, i|
  importance = or_query_weight[i]
  if (importance < 2)
    "redis.call('sismember', '#{or_query}', id)"
  else
    "redis.call('sismember', '#{or_query}', id) * #{importance.to_i}"
  end
end
or_queries_score << "((monthly_rent <= #{nice_to_have_max_monthly_rent}) and #{nice_to_have_max_monthly_rent_weight} or 0)"
or_queries_score << "((square_feet  >= #{nice_to_have_min_square_feet}) and #{nice_to_have_min_square_feet_weight} or 0)"
...
-- Could use dp to determine which scores are considered valid, and/or compress the score into a bitset
while (iterator >= 0) do
...
for i = 1, #ids do
   local id = ids[i]
   -- Geo query will go here, I believe

   local square_feet = tonumber(redis.call('hget', 'square_feet', id))
   if #{min_square_feet} <= square_feet and square_feet <= #{max_square_feet} then
      local monthly_rent = tonumber(redis.call('hget', 'monthly_rent', id))
      if #{min_monthly_rent} <= monthly_rent and monthly_rent <= #{max_monthly_rent} then
...
-- TO CONSIDER: less work could be done if we moved the smaller lists into the largest list, but the constant that might incur could cost more. The algorithm would simply be append if lower score than longest list, prepend if higher score - do nothing if longest list.
