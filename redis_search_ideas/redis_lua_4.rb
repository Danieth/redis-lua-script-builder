and_queries     = %w(amenity:park amenity:hoa amenity:dogs_allowed)
or_queries      = %w(amenity:cats_allowed amenity:ski_resort amenity:parking_garage)
or_query_weight = [1, 3, 1]
max_score       = or_query_weight.inject(&:+)
...
for i = 1, #ids do
   ...
   # Other ideas, use standard keys and try to sort the array based on those keys. Right now this is essentially O(n*log(n)) because zrank is expensive.

   local square_feet = redis.call('zrank', 'square_feet', id)
   if #{min_square_feet} <= square_feet and square_feet <= #{max_square_feet} then
      local monthly_rent = redis.call('zrank', 'monthly_rent', id)
